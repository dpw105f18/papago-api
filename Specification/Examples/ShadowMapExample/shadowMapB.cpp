...
TextureRendrer texRend = TextureRendrer(); //#
...
while(true){
	mainPass.dependsOn(shadowPass); //#
	screenRend.SetRenderpass(mainPass); //#
	texRend.SetRenderpass(mainPass); //#
	...
	auto tex = texRend.render(mainPass.output["outColor"]); //#
}